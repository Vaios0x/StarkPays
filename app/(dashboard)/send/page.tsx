...
export default function SendPage() {
  const { sendMoney, isLoading, isInitialized } = useChipiPay();
  ...
  const handleFraudCheck = async () => {
    ...
    try {
      ...
      if (data.blocked) {
        toast.error("⚠️ Transacción bloqueada por seguridad", {
          description: "Patrón sospechoso detectado. Contacta soporte.",
        });
      } else {
        setStep(2);
        toast.success("✓ Análisis completado", {
          description: `Score de seguridad: ${100 - data.riskScore}/100`,
        });
      }
    } catch (error) {
  ...
  const handleSend = async () => {
    if (!isInitialized) {
      toast.error("Chipi Pay no está inicializado");
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        setStep(3);
        toast.success("✅ Dinero enviado exitosamente", {
          description: `Hash: ${result.transactionHash?.slice(0, 10)}...`,
        });
      } else {
        toast.error("❌ Error enviando dinero", {
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        >
          <div className="inline-flex items-center space-x-3 glass-card mb-8">
            ...
            <span className="text-sm font-medium text-dark-200">Sistema Neural Activo</span>
            <div className="w-2 h-2 bg-neural-400 rounded-full neural-pulse"></div>
          </div>
          
          <h1 className="text-5xl font-bold text-dark-100 mb-6">
            Enviar <span className="neural-text">Dinero Neural</span>
          </h1>
          <p className="text-xl text-dark-300 max-w-2xl mx-auto">
            Protegido por IA neural • Comisión 0.5% • Llegada en 5 minutos
          </p>
        </motion.div>
