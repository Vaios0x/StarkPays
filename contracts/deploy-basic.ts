...
console.log("🚀 Desplegando contratos REALES con configuración básica...");
console.log(`💰 Cuenta: ${ACCOUNT_ADDRESS}`);

const provider = new RpcProvider({ nodeUrl: RPC_URL });
const account = new Account(provider, ACCOUNT_ADDRESS, PRIVATE_KEY);

// Función para desplegar un contrato básico
async function deployBasicContract(contractName: string, constructorCalldata: any[]) {
  try {
    ...
    // Declarar contrato sin opciones adicionales
    ...
    // Deploy contrato sin opciones adicionales
...
async function main() {
  try {
    console.log("🎯 Iniciando deployment REAL básico en Starknet Sepolia...\n");
    
    const contracts = [];
    
    // 1. PaymentProcessor
    console.log("📦 Desplegando PaymentProcessor...");
    const paymentProcessor = await deployBasicContract("PaymentProcessor", [
    ...
    const remesaVault = await deployBasicContract("RemesaVault", [
      ACCOUNT_ADDRESS, // owner
    ...
    const tandaSavings = await deployBasicContract("TandaSavings", [
      ACCOUNT_ADDRESS, // owner
    ]);
    ...
    const config = {
      ...
      deployment: {
        ...
        method: "starknet_js_basic_deployment",
        network: "sepolia",
        version: "auto"
      }
    };
