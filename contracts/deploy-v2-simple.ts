...
console.log("🚀 Desplegando contratos REALES con transacciones V2...");
console.log(`💰 Cuenta: ${ACCOUNT_ADDRESS}`);

const provider = new RpcProvider({ nodeUrl: RPC_URL });
const account = new Account(provider, ACCOUNT_ADDRESS, PRIVATE_KEY);

// Función para desplegar un contrato con V2
async function deployV2Contract(contractName: string, constructorCalldata: any[]) {
  try {
    ...
    // Declarar contrato con V2
    const declareResponse = await account.declare({
      contract: sierraArtifact,
      casm: casmArtifact,
    }, {
      version: "0x2", // Usar versión 2
...
async function main() {
  try {
    console.log("🎯 Iniciando deployment REAL con V2 en Starknet Sepolia...\n");
    
    const contracts = [];
    
    // 1. PaymentProcessor
    console.log("📦 Desplegando PaymentProcessor...");
    const paymentProcessor = await deployV2Contract("PaymentProcessor", [
    ...
    const remesaVault = await deployV2Contract("RemesaVault", [
      ACCOUNT_ADDRESS, // owner
    ...
    const tandaSavings = await deployV2Contract("TandaSavings", [
      ACCOUNT_ADDRESS, // owner
    ]);
    ...
    const config = {
      ...
      deployment: {
        ...
        method: "starknet_js_v2_deployment",
        network: "sepolia",
        version: "v2"
      }
    };
