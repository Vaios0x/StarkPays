...
console.log("🎯 Desplegando contratos Starkpays con manejo robusto de fees...");
...
async function deployContract(contractName: string, constructorCalldata: any[]) {
  try {
    ...
    if (!fs.existsSync(sierraPath) || !fs.existsSync(casmPath)) {
      throw new Error(`Archivos de artefactos no encontrados para ${contractName}`);
    }
    ...
    // Declarar contrato con fee fijo robusto
    console.log(`📋 Declarando ${contractName}...`);
    
    try {
      // Intentar con estimación automática primero
      ...
      console.log(`⚠️  Error con estimación automática, usando fee fijo...`);
      
      // Usar fee fijo como fallback
      const declareResponse = await account.declare({
        contract: sierraArtifact,
        casm: casmArtifact,
      }, {
        maxFee: cairo.uint256(5000000000000000) // 0.005 ETH
      });
...
async function main() {
  try {
    console.log("🎯 Iniciando deployment robusto de contratos...\n");
    ...
    fs.writeFileSync("contracts-deployed.json", JSON.stringify(config, null, 2));
    
    console.log("\n🎉 ¡Deployment robusto completado exitosamente!");
    ...
    console.log("1. 📱 Usar Remix IDE: https://remix.ethereum.org/");
    console.log("2. 🔧 Usar Starkli: cargo install starkli");
    console.log("3. 🌐 Usar Starkscan: https://sepolia.starkscan.co/");
  }
}
