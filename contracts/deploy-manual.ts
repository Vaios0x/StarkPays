...
console.log("🎯 Desplegando contratos con configuración manual...");
console.log(`💰 Cuenta: ${ACCOUNT_ADDRESS}`);

const provider = new RpcProvider({ nodeUrl: RPC_URL });
const account = new Account(provider, ACCOUNT_ADDRESS, PRIVATE_KEY);

async function deployContractManual(contractName: string, constructorCalldata: any[]) {
  try {
    console.log(`\n🚀 Desplegando ${contractName} con configuración manual...`);
    ...
    // Declarar contrato con configuración manual completa
    const declareResponse = await account.declare({
      contract: sierraArtifact,
      casm: casmArtifact,
    }, {
      version: "0x1", // Usar versión 1 (más estable)
    ...
    // Deploy contrato con configuración manual
...
async function main() {
  try {
    console.log("🎯 Iniciando deployment con configuración manual...\n");
    
    const contracts = [];
    
    // 1. PaymentProcessor
    console.log("📦 Desplegando PaymentProcessor...");
    const paymentProcessor = await deployContractManual("PaymentProcessor", [
    ...
    const remesaVault = await deployContractManual("RemesaVault", [
      ACCOUNT_ADDRESS, // owner
    ...
    const tandaSavings = await deployContractManual("TandaSavings", [
      ACCOUNT_ADDRESS, // owner
    ]);
    ...
    fs.writeFileSync("contracts-deployed-manual.json", JSON.stringify(config, null, 2));
    
    console.log("\n🎉 ¡Deployment con configuración manual completado exitosamente!");
    ...
    console.log("\n🔧 Próximos pasos:");
    console.log("1. ✅ Contratos desplegados con configuración manual");
    ...
    console.error("❌ Error durante el deployment con configuración manual:", error);
