...
console.log("🚀 Desplegando contratos REALES con configuración simple...");
console.log(`💰 Cuenta: ${ACCOUNT_ADDRESS}`);

const provider = new RpcProvider({ nodeUrl: RPC_URL });
const account = new Account(provider, ACCOUNT_ADDRESS, PRIVATE_KEY);

// Función para desplegar un contrato con configuración simple
async function deploySimpleContract(contractName: string, constructorCalldata: any[]) {
  try {
    ...
    // Declarar contrato con configuración simple
    ...
    // Deploy contrato con configuración simple
...
async function main() {
  try {
    console.log("🎯 Iniciando deployment REAL simple en Starknet Sepolia...\n");
    
    const contracts = [];
    
    // 1. PaymentProcessor
    console.log("📦 Desplegando PaymentProcessor...");
    const paymentProcessor = await deploySimpleContract("PaymentProcessor", [
    ...
    const remesaVault = await deploySimpleContract("RemesaVault", [
      ACCOUNT_ADDRESS, // owner
    ...
    const tandaSavings = await deploySimpleContract("TandaSavings", [
      ACCOUNT_ADDRESS, // owner
    ]);
    ...
    const config = {
      ...
      deployment: {
        ...
        method: "starknet_js_simple_deployment",
        network: "sepolia",
        version: "v3"
      }
    };
